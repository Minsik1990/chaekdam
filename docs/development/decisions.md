...
## ADR-011: 포레스트그린 디자인 시스템

- **날짜**: 2026-02-07
- **상태**: 확정
- **결정**: 밍들레씨 캐릭터 테마를 제거하고, 포레스트그린 테마 디자인 시스템으로 전환
- **근거**:
  - 밍들레씨 캐릭터 테마(그린/노랑/핑크)가 "독서 기록" 서비스에 과하게 캐릭터 의존적
  - 포레스트그린(#2D6A4F) 포인트 + 깔끔한 미니멀 디자인이 개인 기록 서비스에 적합
  - 토스의 검증된 UX 패턴 (하단 탭, 카드 레이아웃, 타이포 중심) 활용
  - 캐릭터 에셋 제작 부담 제거 -> 개발 속도 향상
  - 라이트 모드 전용으로 범위 축소 -> 복잡도 감소
- **변경 내용**:
  - Primary: #7CB342 (그린) -> #2D6A4F (포레스트그린)
  - Accent: #FFD54F (노랑) -> #C9A96E (크래프트베이지)
  - 캐릭터 일러스트 -> Lucide 아이콘
