...
### 선택 (Phase 2+)
- [Cloudflare](https://cloudflare.com) - R2 이미지 저장
- [알라딘 OpenAPI](https://blog.aladin.co.kr/openapi) - 도서 상세 정보
- [카카오 개발자](https://developers.kakao.com) - 카카오 로그인
...
### 방법 2: 수동 설정
```bash
npx create-next-app@latest mingdle --typescript --tailwind --eslint --app --use-pnpm
cd mingdle
...
```bash
# shadcn/ui 초기화 (Maia 스타일 선택)
npx shadcn@latest init

# 필수 컴포넌트 추가
npx shadcn@latest add button card input textarea dialog dropdown-menu
npx shadcn@latest add calendar avatar badge separator tabs
```
...
### 마이그레이션 적용
```bash
# Supabase MCP로 생성하거나 수동 작성
npx supabase migration new initial_schema
# supabase/migrations/ 에 SQL 작성
